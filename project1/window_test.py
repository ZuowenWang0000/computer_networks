...
def no_answer_process(log_file, reliable_no_answer_filename, rno_port, rel_port, window_size):
    try:
        command = "%s -d -w %d %d localhost:%d" % (reliable_no_answer_filename, window_size, rno_port, rel_port)
...
def student_process(reliable_filename, rno_port, rel_port, window_size):
    try:
        command = "%s -w %d -d %d localhost:%d" % (reliable_filename, window_size, rel_port, rno_port)
